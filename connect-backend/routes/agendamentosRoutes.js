...
router.post('/agendar-monitoria', async (req, res) => {
  try {
    ...
    if (!alunoId || !monitorId || !data) {
      return res.status(400).json({ mensagem: 'Dados incompletos para agendamento' });
    }

    // Verifica se IDs são válidos
    if (!mongoose.Types.ObjectId.isValid(alunoId) || !mongoose.Types.ObjectId.isValid(monitorId)) {
    ...
    if (conflito) {
      return res.status(400).json({ mensagem: 'Horário indisponível para este monitor.' });
    }
    ...
    res.status(500).json({ mensagem: 'Erro ao agendar monitoria', erro: err.message });
  }
});
